...
def main():
    ...
    variants = json.loads(Path(__file__).with_name("display-variants.json").read_text())
    variant = os.environ.get("CARPLAY_DISPLAY_VARIANT", "default")
    if variant not in variants:
        raise RuntimeError("Choose a known display_variant: default or wide.")
    requested = {"id": variant, **variants[variant]}
