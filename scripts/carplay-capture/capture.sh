...
# The workflow checks out the explicitly selected app commit into screenshot-app and
# successfully connects the real external CarPlay display before reaching here.
app_commit="${CARPLAY_APP_COMMIT:?Expected full app commit is required}"
capture_mode="${CARPLAY_CAPTURE_MODE:-results}"
...
cp scripts/carplay-capture/LayoutProfileRepositoryTests.swift \
...
fixture_sha = hashlib.sha256(pathlib.Path('scripts/carplay-capture/LayoutProfileRepositoryTests.swift').read_bytes()).hexdigest()
...
source = {
    ...
    'profileFixtureSHA256': hashlib.sha256(pathlib.Path('scripts/carplay-capture/LayoutProfileRepositoryTests.swift').read_bytes()).hexdigest(),
...
if [ "$capture_mode" = results ]; then
  CARPLAY_SCREEN_TEXT="$RUNNER_TEMP/nextstop-screen-text" \
    python3 scripts/carplay-capture/layout.py
else
